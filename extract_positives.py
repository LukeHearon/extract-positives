...
def _process_recordings(
    ...
    group_clips: dict[str, list[np.ndarray]] = defaultdict(list)
    ...
    for ident, mp3_path, frames_df, file_dt in recordings:
        ...
        window_starts = window_df["start"]
        segments = _merge_segments(sorted(window_starts.tolist()))
        print(f"  {len(window_starts)} frame(s) → {len(segments)} segment(s)")
        ...
        for seg_s, seg_e in segments:
            ...
            print(f"  clip {seg_s:.2f}s – {seg_e:.2f}s")
            try:
                track.seek(seek)
                samples = track.read(n)
                if len(samples) < n:
                    samples = np.concatenate([samples, np.zeros(n - len(samples))])
                clips.append(np.concatenate([samples, silence]))
            except Exception as e:
                print(f"  error at {seg_s:.2f}s: {e}")
        ...
        if cfg.join_mode == "file":
            ...
            sf.write(str(out_file), np.concatenate(clips), SAMPLERATE)
            print(f"  Written: {out_file}")
            written += 1
        else:
            group_clips[key].extend(clips)
    ...
    if cfg.join_mode == "file":
        ...
        if not group_clips:
            ...
            return
        print()
        for key, clips in group_clips.items():
            out_file = group_outpath[key]
            out_file.parent.mkdir(parents=True, exist_ok=True)
            sf.write(str(out_file), np.concatenate(clips), SAMPLERATE)
            print(f"Written: {out_file}")
